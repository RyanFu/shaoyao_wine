...
module.exports = class extends Base {
  ...
  async getCart() {
    ...
    for (const cartItem of cartList) {
      ...
      // 查找商品的图片
      cartItem.list_pic_url = await this.model('goods').where({
        id: cartItem.goods_id
      }).getField('primary_pic_url', true);
    }
  ...
  async addAction() {
    ...
    if (think.isEmpty(cartInfo)) {
      ...
      const cartData = {
        ...
        goods_specifition_ids: '', //goodsInfo.goods_specification_ids,
        checked: 1
      };

      await this.model('cart').thenAdd(cartData, {
        product_id: productId
      });
    } else {
      // 如果已经存在购物车中，则数量增加
  ...
  async checkoutAction() {
    ...
    let demo = {
      uid: think.userId,
      addressId: addressId,
      iiischeckedAddress: !think.isEmpty(checkedAddress),
