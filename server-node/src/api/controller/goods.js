...
module.exports = class extends Base {
  ...
  async detailAction() {
    ...
    info.list_pic_url = JSON.parse(info.list_pic_url);

    info.goods_desc = JSON.parse(info.goods_desc);;

    let category_id = [];
    let gcids = info.category_id.substring(1);
  ...
  async listAction() {
    ...
    if (!think.isEmpty(categoryId)) {
      whereMap.category_id = ['like', '%@' + categoryId + '@%'];
    }
    // 排序
    let orderMap = {};
    if (sort === 'price') {
  ...
  async relatedAction() {
    ...
    if (think.isEmpty(relatedGoodsIds)) {
      ...
      }).field(['id', 'name', 'primary_pic_url', 'retail_price']).limit(8).select();
    } else {
      relatedGoods = await model.where({
        id: ['IN', relatedGoodsIds]
      }).field(['id', 'name', 'primary_pic_url', 'retail_price']).select();
    }
